...
self.addEventListener("activate", (e) => {
  ...
  const d = (k) => caches.delete(k);
  const f = (ks) => Promise.all(ks.filter((k) => k !== name).map(d));
  e.waitUntil(caches.keys().then(f));
});
  ...
  const res = caches.open(name).then((c) => {
    return caches.match(e.request).then((response) => {
      const network = fetch(e.request).then(function (response) {
        if (e.request.method === "GET") c.put(e.request, response.clone());
        return response;
      });
